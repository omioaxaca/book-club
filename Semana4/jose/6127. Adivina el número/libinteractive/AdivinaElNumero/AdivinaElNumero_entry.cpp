...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> AdivinaElNumero.adivina
				uint32_t __cookie;
				long long adivina_a;
				readfull(&__in, &adivina_a, sizeof(long long), 1);
				long long adivina_b;
				readfull(&__in, &adivina_b, sizeof(long long), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				adivina(adivina_a, adivina_b);
...
long long pista(long long x) {
	...
	writefull(&__out, &__msgid, sizeof(__msgid));
	writefull(&__out, &x, sizeof(long long));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "./AdivinaElNumero_in", O_RDONLY);
	openstream(&__out, "./AdivinaElNumero_out", O_WRONLY);
