...
static struct __stream __AdivinaElNumero_in, __AdivinaElNumero_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__AdivinaElNumero_in, "./AdivinaElNumero_in", O_WRONLY);
	openstream(&__AdivinaElNumero_out, "./AdivinaElNumero_out", O_RDONLY);
}
...
static void __message_loop_AdivinaElNumero(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__AdivinaElNumero_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x71651df7u: {
				// AdivinaElNumero -> Main.pista
				uint32_t __cookie;
				long long pista_x;
				readfull(&__AdivinaElNumero_out, &pista_x, sizeof(long long), 1);
				readfull(&__AdivinaElNumero_out, &__cookie, sizeof(__cookie), 1);

				long long result =
				pista(pista_x);

				writefull(&__AdivinaElNumero_in, &__msgid, sizeof(__msgid));
				writefull(&__AdivinaElNumero_in, &result, sizeof(result));
				writefull(&__AdivinaElNumero_in, &__cookie, sizeof(__cookie));
				writeflush(&__AdivinaElNumero_in);
				break;
			}
...
void adivina(long long a, long long b) {
	...
	writefull(&__AdivinaElNumero_in, &__msgid, sizeof(__msgid));
	writefull(&__AdivinaElNumero_in, &a, sizeof(long long));
	writefull(&__AdivinaElNumero_in, &b, sizeof(long long));
	writefull(&__AdivinaElNumero_in, &__cookie, sizeof(__cookie));
	writeflush(&__AdivinaElNumero_in);

	__message_loop_AdivinaElNumero(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__AdivinaElNumero_out, &__cookie_result, sizeof(__cookie_result), 1);
